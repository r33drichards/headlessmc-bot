...
Headless Minecraft bot for 2b2t using HeadlessMC + Fabric 1.21.4 with Meteor Client and Baritone. Runs Minecraft without a real display via Xvfb, with optional VNC/noVNC for visual monitoring.
...
- **`Dockerfile`** — Based on `eclipse-temurin:21-jammy` (multi-arch, ARM64 native). Downloads HeadlessMC wrapper, sets up Xvfb + VNC + noVNC, downloads Meteor Client and Baritone for MC 1.21.4, configures HeadlessMC to use Xvfb instead of LWJGL patching.
...
download fabric:1.21.4                  # Download Minecraft + Fabric
...
launch fabric:1.21.4 -lwjgl            # Launch headless (local, no Xvfb)
launch fabric:1.21.4                    # Launch with Xvfb (Docker)
